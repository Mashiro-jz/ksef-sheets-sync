...
async function authenticateKsef(nipFirmy) {
  ...
  if (!certificateLines) {
    throw new Error(
      'KSeF: nie udało się przygotować certyfikatu PEM.'
    );
  }
  ...
  if (!referenceNumber) {
    throw new Error(
      'KSeF Auth: brak referenceNumber w odpowiedzi.'
    );
  }

  if (!authenticationToken) {
    throw new Error(
      'KSeF Auth: brak authenticationToken.token w odpowiedzi.'
    );
  }
  ...
  if (!accessToken) {
    throw new Error(
      'KSeF Token Redeem: brak accessToken.token w odpowiedzi.'
    );
  }
...
export async function POST(request) {
  try {
    ...
    const nipFirmy = (process.env.NIP_FIRMY || '')
      .replace(/\D/g, '');

    if (!nipFirmy) {
      throw new Error(
        'Brak lub nieprawidłowy NIP_FIRMY.'
      );
    }
    ...
    try {
      ...
      console.warn(
        'Nie udało się pobrać historii faktur:',
        e.message
      );
    }
    ...
    console.log(
      `Rozpoczynam uwierzytelnianie KSeF dla NIP: ${nipFirmy}`
    );

    const accessToken = await authenticateKsef(
      nipFirmy
    );

    console.log(
      'Uwierzytelnianie KSeF zakończone sukcesem.'
    );
    ...
    const poczatekMiesiacaISO =
      poczatekMiesiaca.toISOString();
    ...
    console.log(
      `KSeF: pobrano ${allInvoices.length} faktur.`
    );
    ...
    for (const inv of allInvoices) {
      ...
      //
      // Do deduplikacji używamy numeru faktury z kolumny H,
      // zgodnie z dotychczasową strukturą arkusza.
      ...
      if (!invoiceNumber) {
        console.warn(
          'Pominięto fakturę bez invoiceNumber:',
          inv.ksefNumber
        );
      ...
      const miesiac =
        dataFaktury.length >= 7
          ? dataFaktury.substring(5, 7)
          : '';
      ...
      // Nazwa wystawcy
      //
      // Przy fakturach kosztowych interesuje nas sprzedawca.
      // -----------------------------------------------------
      ...
      // Dodajemy od razu do Set,
      // aby uniknąć duplikatów również wtedy,
      // gdy KSeF zwróci dwa rekordy z tym samym numerem.
